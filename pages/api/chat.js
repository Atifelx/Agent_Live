...
const MODEL_CONFIG = {
  openrouter: {
    ...
    fallback2: 'meta-llama/llama-3.2-3b-instruct:free',
  },
  gemini: {
...
/**
 * Call Google Gemini API (simplified - no streaming complexity)
 */
async function callGeminiAPI(messages) {
  console.log('🛡️ Using Gemini fallback...');
...
async function callModelWithFallback(messages, options = {}) {
  ...
  // If already using Gemini, continue with it
  ...
  for (let i = 0; i < models.length; i++) {
    try {
      ...
      console.log(`✅ ${models[i]} worked`);
      return response;

    } catch (error) {
      console.error(`❌ ${models[i]} failed:`, error.message);

      // Last model failed, try Gemini
      if (i === models.length - 1) {
        console.log('⚠️ All OpenRouter failed, switching to Gemini...');
        try {
          USE_GEMINI_FALLBACK = true;
          const geminiResponse = await callGeminiAPI(messages);

          // If streaming was requested, we need to fake it
          if (options.stream) {
          ...
          throw new Error(`All failed. OpenRouter: ${error.message}, Gemini: ${geminiError.message}`);
        }
      }
    }
  }
...
/**
 * Create fake streaming from Gemini text response
 */
function createFakeStream(text) {
      ...
      return {
        async next() {
          if (index < words.length) {
            ...
            const content = word + (index < words.length ? ' ' : '');
            return {
              done: false,
              value: {
...
async function searchDocuments(query) {
  try {
    ...
    console.log('📚 Searching docs:', query);
    ...
    const seenTexts = new Set();
    for (const res of results) {
      const normalized = res.text.substring(0, 100).toLowerCase().replace(/\s/g, '');
...
async function processWithAgent(userMessage, chatHistory = [], activeDocs = [], onStream = () => { }) {
  const libraryContext = activeDocs.length > 0
    ? `Active documents: [${activeDocs.join(', ')}].`
    : "Library is empty.";

  const systemPrompt = `You are Clever Chat, an AI research partner.

${libraryContext}

Tools:
1. searchDocuments(query) - Search uploaded files
2. searchWeb(query) - Search live web

Rules:
- For document questions, use searchDocuments
- For current events, use searchWeb
- For greetings, answer directly
- To use tool: TOOL: <toolName>\\nQUERY: <query>
- After tool results, provide final answer
  ...
  while (turns < maxTurns) {
    ...
    try {
      ...
      if (toolMatch && queryMatch) {
        ...
        const label = toolName === 'searchDocuments' ? 'Searching library' : 'Searching web';
        onStream('thought', `${label}: "${searchQuery}"`);
        ...
        if (toolResult && toolResult.success) {
          ...
          currentMessages.push({
            role: 'user',
            content: `TOOL RESULTS:\n${toolResult.context}\n\nProvide final answer. Be concise.`
          });

          if (toolResult.results) sources.push(...toolResult.results.map(r => r.source));
          if (toolResult.sources) sources.push(...toolResult.sources);
        } else {
          currentMessages.push({ role: 'assistant', content: agentResponse });
          currentMessages.push({ role: 'user', content: `Tool failed. Answer from knowledge.` });
        }
      } else {
        // No tool - final answer
        onStream('thought', 'Generating answer...');
        onStream('sources', [...new Set(sources)].join(','));

        const finalStream = await callModelWithFallback([
          ...currentMessages,
          { role: 'system', content: "Final answer. Be concise, accurate." }
        ], {
          temperature: 0.5,
          presence_penalty: 0.4,
          frequency_penalty: 0.5,
      ...
      onStream('thought', 'Retrying...');

      if (error.message.includes('All failed')) {
        onStream('answer', `Error: ${error.message}`);
        return;
      }
    }
  }

  onStream('answer', 'Please rephrase your question.');
}
